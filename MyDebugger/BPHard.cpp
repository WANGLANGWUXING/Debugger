...
bool BPHard::Install()
{
	...
	else if (ct.Dr3 == 0)
	{
		...
		return false;
	SetThreadContext(m_dbgObj.m_hCurrThread, &ct);
	m_bOnce = false;
	return true;
}
...
bool BPHard::IsMe(const EXCEPTION_DEBUG_INFO & ExcDebInf) const
{
	...
	pDr7 = (PDBG_REG7)&ct.Dr7;
	// 通过DR6寄存器的BX 判断是哪个硬件断点
	if (pDr6->B0)
	{
		// 判断DR7寄存器的RWX 等于0 为执行时中断
		if (pDr7->RW0 == 0)
		{
			// 判断断点类型，如果是breakpointType_hard_e ，那么比较异常地址
			// 否则不是当前断点
			if (m_eType == breakpointType_hard_e)
		...
		// RWX 不等于 0时 断点类型也不是硬件断点执行时中断
		// 判断断点类型是否不等于breakpointType_hard_e
